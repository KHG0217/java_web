...
@WebServlet("/fortune")
public class ForturnServlet extends HttpServlet {

	@Override
						//ServletRequest 부모
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		//1. 오늘의 운세를 얻어오는 비지니스 로직을 수행했다고 가정하자.
		 ...
		 * 
		 * 	.getRequestDispatcher("요청을 전달받을 webapp 안에 있는 jsp 페이지의 위치")
		 * 
		 * 	메소드를 이용해서 얻어낼 수 있다.
		 */
		RequestDispatcher rd=req.getRequestDispatcher("/test/fortune.jsp");
		
		//지정된 jsp 페이지로 응답을 대신하라고 위임한다.
		rd.forward(req, resp);
	}
}
